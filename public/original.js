...
function redirectToMap() {
    ...
    if (code.length === 6 && !isNaN(code)) { // Check if the entered code is exactly 6 digits and numeric
        window.location.replace("https://projects.audioenhancement.com/project/" + code + "/map");
    } else {
...
function openBOM() {
    var code = document.getElementById('codeInput').value;
    if (code.length === 6 && !isNaN(code)) { // Check if the entered code is exactly 6 digits and numeric
        window.location.replace("https://projects.audioenhancement.com/project/" + code + "/bom");
    } else {
        // If not 6 digits or empty, perform a Google search instead
        if (code.trim() !== "") {
            window.location.replace("https://www.google.com/search?q=" + encodeURIComponent(code));
